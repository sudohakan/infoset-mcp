...
This server authenticates with the Infoset API using an API key. The key is sensitive and must be handled with care:

- **Never commit credentials to version control.** The `.env` file is included in `.gitignore` by default.
- **Use environment variables or MCP client config** to pass the API key at runtime. The `.env.example` file contains placeholder values only.
...
2. Email the maintainer directly at **sudohakan@users.noreply.github.com** with:
